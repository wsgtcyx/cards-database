import { Card } from "../../../interfaces";
import Set from "../Mythical Island";
const card: Card = {
    set: Set,
    image: {
        de: "https://game.pokemontcgpocket.app/de/tcgp/A1a/005",
        es: "https://game.pokemontcgpocket.app/es/tcgp/A1a/005",
        fr: "https://game.pokemontcgpocket.app/fr/tcgp/A1a/005",
        it: "https://game.pokemontcgpocket.app/it/tcgp/A1a/005",
        "pt-br": "https://game.pokemontcgpocket.app/pt/tcgp/A1a/005",
        "zh-tw": "https://game.pokemontcgpocket.app/zh-tw/tcgp/A1a/005",
        en: "https://game.pokemontcgpocket.app/en/tcgp/A1a/005"
    },
    name: {
        en: "Servine",
        fr: "Lianaja",
        es: "Servine",
        it: "Servine",
        de: "Efoserp",
        'pt-br': "Servine",
        ko: "샤비",
        "zh-tw": "青藤蛇",
    },
    illustrator: "Shigenori Negishi",
    category: "Pokemon",
    hp: 80,
    types: ["Grass"],
    evolveFrom: {
        en: "Snivy",
        "fr": "Snivy",
        "es": "snivy",
        "it": "Snivy",
        "de": "Snivy",
        "pt-br": "Snivy",
        "zh-tw": "藤藤蛇",
    },
    description: {
        en: "It moves along the ground as if sliding. Its swift movements befuddle its foes, and it then attacks with a vine whip.",
        fr: "Il court comme s'il glissait sur le sol. Il déroute l'ennemi\npar ses mouvements et l'assomme d'un coup de liane.",
        es: "Parece que se desliza al correr. Engaña a sus rivales\ncon su velocidad y los fustiga con su látigo.",
        it: "Corre quasi scivolando sulle superfici. Confonde il nemico\ncon i rapidi movimenti, per poi attaccarlo con una frustata.",
        de: "Huscht beinahe gleitend über den Boden\nund täuscht Gegner mit agilen Manövern,\nbis es mithilfe seiner Efeurute obsiegt.",
        'pt-br': "Move-se pelo chão como se estivesse deslizando.\nAtordoa seus inimigos com movimentos rápidos\ne os ataca com um chicote de vinha.",
        ko: "땅을 미끄러지듯 달린다.\n빠른 움직임으로 상대를 혼란시키고\n덩굴채찍으로 꼼짝 못하게 한다.",
        "zh-tw": "它沿著地面移動，就像滑動一樣。它敏捷的動作迷惑了敵人，然後用藤鞭攻擊。"
    },
    stage: "Stage1",
    attacks: [{
            name: {
                en: "Vine Whip",
                fr: "Fouet Lianes",
                es: "Látigo Cepa",
                it: "Frustata",
                de: "Rankenhieb",
                'pt-br': "Chicote de Vinha",
                ko: "덩굴채찍",
                "zh-tw": "藤鞭"
            },
            damage: 50,
            cost: ["Grass", "Colorless"]
        }],
    weaknesses: [{
            type: "Fire",
            value: "+20"
        }],
    retreat: 1,
    rarity: "Two Diamond"
};
export default card;
