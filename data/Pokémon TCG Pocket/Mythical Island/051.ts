import { Card } from "../../../interfaces";
import Set from "../Mythical Island";
const card: Card = {
    set: Set,
    image: {
        de: "https://game.pokemontcgpocket.app/de/tcgp/A1a/051",
        es: "https://game.pokemontcgpocket.app/es/tcgp/A1a/051",
        fr: "https://game.pokemontcgpocket.app/fr/tcgp/A1a/051",
        it: "https://game.pokemontcgpocket.app/it/tcgp/A1a/051",
        "pt-br": "https://game.pokemontcgpocket.app/pt/tcgp/A1a/051",
        "zh-tw": "https://game.pokemontcgpocket.app/zh-tw/tcgp/A1a/051",
        en: "https://game.pokemontcgpocket.app/en/tcgp/A1a/051"
    },
    name: {
        en: "Purrloin",
        fr: "Chacripan",
        es: "Purrloin",
        it: "Purrloin",
        de: "Felilou",
        'pt-br': "Purrloin",
        ko: "쌔비냥",
        "zh-tw": "扒手貓",
    },
    illustrator: "Kagemaru Himeno",
    category: "Pokemon",
    hp: 60,
    types: ["Darkness"],
    description: {
        en: "It steals things from people just to amuse itself with their frustration. A rivalry exists between this Pokémon and Nickit.",
        fr: "Il vole les biens des personnes rien que pour voir leur\nvisage désemparé. Son plus grand rival est Goupilou.",
        es: "Sustrae las pertenencias de las personas solo para\nverlas pasar apuros. Es un rival encarnizado de Nickit.",
        it: "Ruba le cose alle persone per vederle\nin difficoltà. Nickit è il suo rivale.",
        de: "Es bestiehlt liebend gerne Leute, um sich an ihrer\nFrustration zu erfreuen. Mit Kleptifux verbindet\nes eine tiefe Rivalität.",
        'pt-br': "Furta as pessoas só para se divertir com a frustração delas.\nExiste uma rivalidade entre este Pokémon e Nickit.",
        ko: "곤란해하는 모습을 보기 위해\n사람의 물건을 훔친다.\n훔처우와는 라이벌 관계다.",
        "zh-tw": "它從人們那裡偷東西只是為了取樂他們的挫敗感。該神奇寶貝和尼基特之間存在競爭。"
    },
    stage: "Basic",
    attacks: [{
            name: {
                en: "Scratch",
                fr: "Griffe",
                es: "Arañazo",
                it: "Graffio",
                de: "Kratzer",
                'pt-br': "Arranhão",
                ko: "할퀴기",
                "zh-tw": "划痕"
            },
            damage: 20,
            cost: ["Darkness"]
        }],
    weaknesses: [{
            type: "Grass",
            value: "+20"
        }],
    retreat: 1,
    rarity: "One Diamond"
};
export default card;
