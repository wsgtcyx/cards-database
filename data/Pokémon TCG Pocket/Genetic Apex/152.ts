import { Card } from "../../../interfaces";
import Set from "../Genetic Apex";
const card: Card = {
    set: Set,
    image: {
        de: "https://game.pokemontcgpocket.app/de/tcgp/A1/152",
        es: "https://game.pokemontcgpocket.app/es/tcgp/A1/152",
        fr: "https://game.pokemontcgpocket.app/fr/tcgp/A1/152",
        it: "https://game.pokemontcgpocket.app/it/tcgp/A1/152",
        "pt-br": "https://game.pokemontcgpocket.app/pt/tcgp/A1/152",
        "zh-tw": "https://game.pokemontcgpocket.app/zh-tw/tcgp/A1/152",
        en: "https://game.pokemontcgpocket.app/en/tcgp/A1/152"
    },
    name: {
        en: "Marowak",
        fr: "Ossatueur",
        es: "Marowak",
        it: "Marowak",
        de: "Knogga",
        'pt-br': "Marowak",
        ko: "텅구리",
        "zh-tw": "嘎啦嘎啦",
    },
    illustrator: "Shin Nagasawa",
    category: "Pokemon",
    hp: 100,
    types: ["Fighting"],
    stage: "Stage1",
    evolveFrom: {
        en: "Cubone",
        "fr": "Cubone",
        "es": "cubone",
        "it": "Cubone",
        "de": "Cubone",
        "pt-br": "Cubo",
        "zh-tw": "庫博內"
    },
    attacks: [{
            cost: ["Fighting"],
            name: {
                en: "Bone Beatdown",
                fr: "Dérouillée d'Os",
                es: "Derribo Óseo",
                it: "Colpo d'Osso",
                de: "Knochenprügel",
                'pt-br': "Surra de Osso",
                ko: "뼈때리기",
                "zh-tw": "骨頭擊敗"
            },
            damage: "40"
        }],
    weaknesses: [{
            type: "Grass",
            value: "+20"
        }],
    retreat: 1,
    rarity: "Two Diamond",
    description: {
        en: "This Pokémon overcame its sorrow to evolve a sturdy new body. Marowak faces its opponents bravely, using a bone as a weapon.",
        fr: "Il s'est endurci et a évolué depuis qu'il a réussi\nà surmonter sa peine. Il utilise son os en guise\nd'arme et affronte ses ennemis avec bravoure.",
        es: "Ha evolucionado tras fortalecerse y\nsuperar su pena. Ahora lucha con arrojo\nblandiendo su hueso a modo de arma.",
        it: "Si è evoluto dopo essersi temprato e aver\nsuperato il lutto. Usa l'osso come un'arma\ne lotta valorosamente contro i nemici.",
        de: "Als es seine Trauer überwand, entwickelte es sich\nund wurde stärker. Mit seinem Knochen als Waffe\nstellt sich Knogga mutig seinen Feinden.",
        'pt-br': "Lidou com sua tristeza para evoluir um\ncorpo novo e forte. Enfrenta seus oponentes\ncom bravura, empunhando um osso como arma.",
        ko: "슬픔을 극복하고 늠름하게\n진화했다. 뼈를 무기 삼아\n용감하게 적과 맞서 싸운다.",
        "zh-tw": "這只神奇寶貝克服了悲傷，進化出堅固的新身體。馬羅瓦克以骨頭為武器，勇敢地面對對手。"
    },
    boosters: ["mewtwo"]
};
export default card;
