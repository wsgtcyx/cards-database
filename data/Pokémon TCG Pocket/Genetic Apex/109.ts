import { Card } from "../../../interfaces";
import Set from "../Genetic Apex";
const card: Card = {
    set: Set,
    image: {
        de: "https://game.pokemontcgpocket.app/de/tcgp/A1/109",
        es: "https://game.pokemontcgpocket.app/es/tcgp/A1/109",
        fr: "https://game.pokemontcgpocket.app/fr/tcgp/A1/109",
        it: "https://game.pokemontcgpocket.app/it/tcgp/A1/109",
        "pt-br": "https://game.pokemontcgpocket.app/pt/tcgp/A1/109",
        "zh-tw": "https://game.pokemontcgpocket.app/zh-tw/tcgp/A1/109",
        en: "https://game.pokemontcgpocket.app/en/tcgp/A1/109"
    },
    name: {
        en: "Eelektross",
        fr: "Ohmassacre",
        es: "Eelektross",
        it: "Eelektross",
        de: "Zapplarang",
        'pt-br': "Eelektross",
        ko: "저리더프",
        "zh-tw": "麻麻鰻魚王",
    },
    illustrator: "hatachu",
    category: "Pokemon",
    hp: 140,
    types: ["Lightning"],
    stage: "Stage2",
    evolveFrom: {
        en: "Eelektrik",
        "fr": "Électricité",
        "es": "eléctrico",
        "it": "Elektrik",
        "de": "Elektrik",
        "pt-br": "Elétrico",
        "zh-tw": "電氣技術"
    },
    attacks: [{
            cost: ["Lightning", "Lightning", "Colorless"],
            name: {
                en: "Thunder Fang",
                fr: "Crocs Éclair",
                es: "Colmillo Rayo",
                it: "Fulmindenti",
                de: "Donnerzahn",
                'pt-br': "Presa Trovejante",
                ko: "번개엄니",
                "zh-tw": "雷牙"
            },
            effect: {
                en: "Flip a coin. If heads, your opponent's Active Pokémon is now Paralyzed.",
                fr: "Lancez une pièce. Si c'est face, le Pokémon Actif de votre adversaire est maintenant Paralysé.",
                es: "Lanza 1 moneda. Si sale cara, el Pokémon Activo de tu rival pasa a estar Paralizado.",
                it: "Lancia una moneta. Se esce testa, il Pokémon attivo del tuo avversario viene paralizzato.",
                de: "Wirf 1 Münze. Bei Kopf ist das Aktive Pokémon deines Gegners jetzt paralysiert.",
                ko: "동전을 1번 던져서 앞면이 나오면 상대의 배틀 포켓몬을 마비로 만든다.",
                'pt-br': "Jogue uma moeda. Se sair cara, o Pokémon Ativo do seu oponente agora estará Paralisado.",
                "zh-tw": "拋一枚硬幣。如果正面，則對手的活躍神奇寶貝現在處於麻痺狀態。"
            },
            damage: "80"
        }],
    weaknesses: [{
            type: "Fighting",
            value: "+20"
        }],
    retreat: 3,
    rarity: "Three Diamond",
    description: {
        en: "They crawl out of the ocean using their arms. They will attack prey on shore and immediately drag it into the ocean.",
        fr: "Il peut sortir de l'eau à la force de ses bras\npour attaquer des proies sur la rive et les\nentraîner avec lui dans l'eau en un instant.",
        es: "Sale del mar con la fuerza de sus brazos y ataca a sus\npresas en la costa. Luego se las lleva de vuelta al mar.",
        it: "Striscia fuori dal mare con la sola forza\ndei propri arti. Poi attacca la preda a riva\ne con un guizzo la trascina con sé in acqua.",
        de: "Es kriecht mithilfe seiner Arme an Land,\nstürzt sich auf seine Beute und zerrt sie\naugenblicklich ins Meer.",
        'pt-br': "Rastejam para fora do oceano usando seus braços.\nEles atacam as presas na costa e as arrastam\nimediatamente para a água.",
        ko: "팔 힘으로 바다에서 기어 나와\n물가에 있는 먹이를 덮친다.\n한순간에 바다로 끌고 들어간다.",
        "zh-tw": "他們用手臂從海裡爬出來。它們會攻擊岸上的獵物，然後立即將其拖入大海。"
    },
    boosters: ["mewtwo"]
};
export default card;
