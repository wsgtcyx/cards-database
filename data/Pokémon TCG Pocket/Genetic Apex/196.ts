import { Card } from "../../../interfaces";
import Set from "../Genetic Apex";
const card: Card = {
    set: Set,
    image: {
        de: "https://game.pokemontcgpocket.app/de/tcgp/A1/196",
        es: "https://game.pokemontcgpocket.app/es/tcgp/A1/196",
        fr: "https://game.pokemontcgpocket.app/fr/tcgp/A1/196",
        it: "https://game.pokemontcgpocket.app/it/tcgp/A1/196",
        "pt-br": "https://game.pokemontcgpocket.app/pt/tcgp/A1/196",
        "zh-tw": "https://game.pokemontcgpocket.app/zh-tw/tcgp/A1/196",
        en: "https://game.pokemontcgpocket.app/en/tcgp/A1/196"
    },
    name: {
        en: "Meowth",
        fr: "Miaouss",
        es: "Meowth",
        it: "Meowth",
        de: "Mauzi",
        'pt-br': "Meowth",
        ko: "나옹",
        "zh-tw": "喵喵"
    },
    illustrator: "Mitsuhiro Arita",
    category: "Pokemon",
    hp: 60,
    types: ["Colorless"],
    stage: "Basic",
    attacks: [{
            cost: ["Colorless"],
            name: {
                en: "Pay Day",
                fr: "Jackpot",
                es: "Día de Pago",
                it: "Giornopaga",
                de: "Zahltag",
                'pt-br': "Dia de Pagamento",
                ko: "고양이돈받기",
                "zh-tw": "發薪日"
            },
            effect: {
                en: "Draw a card.",
                fr: "Piochez une carte.",
                es: "Roba 1 carta.",
                it: "Pesca una carta.",
                de: "Ziehe 1 Karte.",
                'pt-br': "Compre 1 carta.",
                ko: "자신의 덱을 1장 뽑는다.",
                "zh-tw": "畫一張卡片。"
            },
            damage: "10"
        }],
    weaknesses: [{
            type: "Fighting",
            value: "+20"
        }],
    retreat: 1,
    rarity: "One Diamond",
    description: {
        en: "All it does is sleep during the daytime. At night, it patrols its territory with its eyes aglow.",
        fr: "Il passe ses journées à dormir. La nuit venue,\nil patrouille sur son territoire, les yeux brillants.",
        es: "Durante el día, se dedica a dormir.\nDe noche, vigila su territorio con un brillo en los ojos.",
        it: "Di giorno non fa che dormire, mentre la notte\nperlustra il suo territorio con gli occhi luccicanti.",
        de: "Es schläft den ganzen Tag. Nachts patrouilliert es\nsein Revier mit glühenden Augen.",
        'pt-br': "Tudo o que este Pokémon faz é dormir\ndurante o dia. À noite, patrulha seu\nterritório com seus olhos brilhantes.",
        ko: "낮에는 거의 잠만 잔다.\n밤이 되면 눈을 반짝이며\n영역을 돌아다닌다.",
        "zh-tw": "它所做的就是在白天睡覺。晚上，它眼睛會發光，在自己的領地巡邏。"
    },
    boosters: ["charizard"]
};
export default card;
