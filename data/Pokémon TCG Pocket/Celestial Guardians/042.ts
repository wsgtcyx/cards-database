import { Card } from "../../../interfaces";
import Set from "../Celestial Guardians";
const card: Card = {
    set: Set,
    image: {
        de: "https://game.pokemontcgpocket.app/de/tcgp/A3/042",
        es: "https://game.pokemontcgpocket.app/es/tcgp/A3/042",
        fr: "https://game.pokemontcgpocket.app/fr/tcgp/A3/042",
        it: "https://game.pokemontcgpocket.app/it/tcgp/A3/042",
        "pt-br": "https://game.pokemontcgpocket.app/pt/tcgp/A3/042",
        "zh-tw": "https://game.pokemontcgpocket.app/zh-tw/tcgp/A3/042",
        en: "https://game.pokemontcgpocket.app/en/tcgp/A3/042"
    },
    name: {
        en: "Shellder",
        fr: "Kokiyas",
        es: "Shellder",
        it: "Shellder",
        de: "Muschas",
        'pt-br': "Shellder",
        ko: "셀러",
        "zh-tw": "大舌貝",
    },
    illustrator: "Midori Harada",
    rarity: "One Diamond",
    category: "Pokemon",
    hp: 70,
    types: ["Water"],
    description: {
        en: "It is encased in a shell that is harder than diamond. Inside, however, it is surprisingly tender.",
        fr: "Une coquille plus dure que le diamant le protège.\nIl est toutefois étonnamment tendre à l'intérieur.",
        es: "Está metido en una concha más dura que el\ndiamante, pero tiene un cuerpo muy blando.",
        it: "La conchiglia esterna è più dura del diamante.\nL'interno, invece, è sorprendentemente morbido.",
        de: "Seine Schale ist härter als Diamant. Im Inneren ist\nes jedoch überraschend weich.",
        'pt-br': "Este Pokémon é envolto em uma concha\nmais dura que diamante. O seu interior,\nporém, é surpreendentemente macio.",
        ko: "다이아몬드보다 단단한\n껍데기에 싸여 있지만\n속은 의외로 말랑하다.",
        "zh-tw": "它被包裹在比鑽石還硬的殼裡。然而，它的內部卻出奇的柔軟。"
    },
    stage: "Basic",
    attacks: [{
            name: {
                en: "Tackle",
                fr: "Charge",
                es: "Placaje",
                it: "Azione",
                de: "Tackle",
                'pt-br': "Investida",
                ko: "몸통박치기",
                "zh-tw": "處理"
            },
            damage: 10,
            cost: ["Water"]
        }],
    weaknesses: [{
            type: "Lightning",
            value: "+20"
        }],
    retreat: 1,
    boosters: ["solgaleo"]
};
export default card;
