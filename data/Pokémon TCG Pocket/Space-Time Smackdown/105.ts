import { Card } from "../../../interfaces";
import Set from "../Space-Time Smackdown";
const card: Card = {
    set: Set,
    image: {
        de: "https://game.pokemontcgpocket.app/de/tcgp/A2/105",
        es: "https://game.pokemontcgpocket.app/es/tcgp/A2/105",
        fr: "https://game.pokemontcgpocket.app/fr/tcgp/A2/105",
        it: "https://game.pokemontcgpocket.app/it/tcgp/A2/105",
        "pt-br": "https://game.pokemontcgpocket.app/pt/tcgp/A2/105",
        "zh-tw": "https://game.pokemontcgpocket.app/zh-tw/tcgp/A2/105",
        en: "https://game.pokemontcgpocket.app/en/tcgp/A2/105"
    },
    name: {
        en: "Skorupi",
        fr: "Rapion",
        es: "Skorupi",
        it: "Skorupi",
        de: "Pionskora",
        'pt-br': "Skorupi",
        ko: "스콜피",
        "zh-tw": "鉗尾蠍",
    },
    illustrator: "Naoki Saito",
    rarity: "One Diamond",
    category: "Pokemon",
    hp: 70,
    types: ["Darkness"],
    description: {
        en: "After burrowing into the sand, it waits patiently for prey to come near. This Pokémon and Sizzlipede share common descent.",
        fr: "Il s'enfouit dans le sable et attend patiemment ses proies.\nIl a des ancêtres en commun avec Grillepattes.",
        es: "Se entierra en la arena y aguarda inmóvil a sus presas.\nSus antepasados están relacionados con los de Sizzlipede.",
        it: "Si nasconde nella sabbia e aspetta immobile la\npreda. Lui e Sizzlipede hanno antenati in comune.",
        de: "Dieses Pokémon vergräbt sich im Sand und\nlauert dort regungslos seiner Beute auf.\nEs ist artverwandt mit Thermopod.",
        'pt-br': "Após se enterrar na areia, espera pacientemente\numa presa se aproximar. Este Pokémon e Sizzlipede\ntêm uma descendência em comum.",
        ko: "모래에 몸을 파묻은 채\n가만히 먹이를 기다린다.\n태우지네와 조상이 가깝다.",
        "zh-tw": "鑽入沙子後，它耐心地等待獵物靠近。這只神奇寶貝和西毛足蟲有著共同的血統。"
    },
    stage: "Basic",
    attacks: [{
            name: {
                en: "Pierce",
                fr: "Transpercement",
                es: "Perforar",
                it: "Perforare",
                de: "Durchbohren",
                'pt-br': "Perfurar",
                ko: "궤찌르기",
                "zh-tw": "刺穿"
            },
            damage: 30,
            cost: ["Darkness", "Colorless"]
        }],
    weaknesses: [{
            type: "Fighting",
            value: "+20"
        }],
    retreat: 2,
    boosters: ["dialga", "palkia"]
};
export default card;
