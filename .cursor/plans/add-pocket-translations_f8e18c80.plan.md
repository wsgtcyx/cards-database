...
- 已完成批次：Mega Rising、Genetic Apex、Triumphant Light、Wisdom of Sea and Sky、Space-Time Smackdown、Shining Revelry、Promos-A、Mythical Island、Extradimensional Crisis、Eevee Grove、Deluxe Pack: ex（均复扫缺口 0，待翻译 0）。
- 翻译映射累计：`scripts/tmp/pocket-translations.json`。
- 近期产物：`scripts/tmp/genetic-missing.json`、`scripts/tmp/genetic-todo.json`、`scripts/tmp/triumph-missing.json`、`scripts/tmp/triumph-todo.json`、`scripts/tmp/wisdom-missing.json`、`scripts/tmp/wisdom-todo.json`、`scripts/tmp/spacetime-missing.json`、`scripts/tmp/spacetime-todo.json`、`scripts/tmp/shining-missing.json`、`scripts/tmp/shining-todo.json`、`scripts/tmp/promos-missing.json`、`scripts/tmp/promos-todo.json`、`scripts/tmp/mythic-missing.json`、`scripts/tmp/mythic-todo.json`、`scripts/tmp/extradim-missing.json`、`scripts/tmp/extradim-todo.json`、`scripts/tmp/eevee-missing.json`、`scripts/tmp/eevee-todo.json`、`scripts/tmp/deluxe-missing.json`、`scripts/tmp/deluxe-todo.json`（当前为空或留档）。
...
- 下一个批次：`data/Pokémon TCG Pocket/Celestial Guardians/**/*.ts`